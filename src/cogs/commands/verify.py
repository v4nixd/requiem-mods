...
class VerifyCommand(commands.Cog):
    ...
    async def verify(self, inter: AppCmdInter, target: Member) -> None:
        ...
        if await self.check_perms(author) == False:
        ...
        if verified_role in target.roles:
            await inter.edit_original_response(
                f"У этого юзера уже есть роль {verified_role.mention}.\nПытаюсь забрать {autorole.mention}"
            )
            await target.remove_roles(
                autorole, reason=f"Verification by {inter.author.id}"
            )
            return

        if autorole not in target.roles:
            await inter.edit_original_response(
                f"У этого юзера уже нету роли {autorole.mention}.\nПытаюсь выдать {verified_role.mention}"
            )
            await target.add_roles(
                verified_role, reason=f"Verification by {inter.author.id}"
            )
            return

        await target.remove_roles(autorole, reason=f"Verification by {inter.author.id}")
        await target.add_roles(
            verified_role, reason=f"Verification by {inter.author.id}"
        )
